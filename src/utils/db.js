...
const fromRow = (row) => ({
  ...
  photoOffsetX: row.photo_offset_x ?? 0,
  photoOffsetY: row.photo_offset_y ?? 0,
...
const toRow = (player) => ({
  ...
  photo_offset_x: player.photoOffsetX ?? 0,
  photo_offset_y: player.photoOffsetY ?? 0,
...
export const db = {
  ...
  getUnassignedPlayers: async () => {
    const { data, error } = await supabase.rpc("get_unassigned_players");
    if (error) throw error;
    return data ?? [];
  },
};
